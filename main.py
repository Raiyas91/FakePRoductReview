...
main = tkinter.Tk()
main.title("Fake Product Review Monitoring System")
...
def Preprocessing():
    ...
    stopwords_s = nltk.corpus.stopwords.words("english")
    vector = TfidfVectorizer(stop_words=stopwords_s, use_idf=True, smooth_idf=False, norm=None, decode_error='replace')
...
def runSVM():
    ...
    rfc = svm.SVC()
    rfc.fit(X, Y)
    predicting = rfc.predict(X_test)
    acc = accuracy_score(y_test, predicting) * 100
...
def predict():
    ...
    test_data = pd.read_csv(testfile, encoding="ISO-8859-1")
    test_data = test_data.values
    text.delete('1.0', END)
    for i in range(len(test_data)):
        msg = str(test_data[i, 0])
        review = msg.lower()
        review = review.strip().lower()
        review = clean_doc(review)
        test_review = vector.transform([review]).toarray()
        predicting = classifier.predict(test_review)
        positive = predicting[0] + 1
        negative = 5 - positive
        text.insert(END, "Review : " + str(test_data[i]) + "\n")
...
title.config(bg='LightSteelBlue3', fg='black')
